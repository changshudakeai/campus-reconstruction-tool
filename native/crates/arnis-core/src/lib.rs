...
pub fn generate_building(request: GenerateBuildingRequest) -> Result<GeneratedBuilding, String> {
    generate_building_with_fault(request, None)
}

pub fn generate_building_with_fault(
    request: GenerateBuildingRequest,
    fault: Option<GenerationFaultPoint>,
) -> Result<GeneratedBuilding, String> {
    fail_generation_at(fault, GenerationFaultPoint::BeforeValidation)?;
    validate_request(&request)?;
    fail_generation_at(fault, GenerationFaultPoint::AfterValidation)?;
    ...
    let generated = sink.finish(GenerationReport {
    ...
    fail_generation_at(fault, GenerationFaultPoint::AfterVoxelization)?;
    Ok(generated)
}

fn fail_generation_at(
    injected: Option<GenerationFaultPoint>,
    point: GenerationFaultPoint,
) -> Result<(), String> {
    if injected == Some(point) {
        Err(format!("injected generation failure at {point:?}"))
    } else {
        Ok(())
    }
}
...
fn validate_request(r: &GenerateBuildingRequest) -> Result<(), String> {
    ...
    if !(0.01..=4.0).contains(&r.blocks_per_meter) {
        return Err("blocksPerMeter must be between 0.01 and 4".into());
    }
...
mod tests {
    ...
    #[test]
    fn accepts_the_schema2_campus_scale_for_retained_detailed_generation() {
        let mut r = request("flat");
        r.blocks_per_meter = 0.02;
        r.components[0].exterior[1].lng = 121.001;
        r.components[0].exterior[2].lng = 121.001;
        r.components[0].exterior[2].lat = 30.999;
        r.components[0].exterior[3].lat = 30.999;
        assert!(generate_building(r).unwrap().report.non_air_blocks > 0);
    }
    ...
    #[test]
    fn every_generation_fault_point_returns_no_generated_building() {
        for point in [
            GenerationFaultPoint::BeforeValidation,
            GenerationFaultPoint::AfterValidation,
            GenerationFaultPoint::AfterVoxelization,
        ] {
            let error = generate_building_with_fault(request("flat"), Some(point))
                .expect_err("an interrupted generation cannot publish a model");
            assert!(
                error.contains(&format!("{point:?}")),
                "the stable failure point must remain diagnosable"
            );
        }
    }
}
