...
.map-panel{position:relative;overflow:hidden}.map-pair{width:100%;height:100%;display:grid;grid-template-columns:1fr 1fr;gap:2px;background:var(--line)}.map{width:100%;height:100%;min-height:430px}.compare-title{position:absolute;z-index:4;top:12px;padding:7px 9px;background:rgba(23,37,31,.9);color:white;border-radius:8px;font-size:10px}.compare-title.left{left:12px}.compare-title.right{left:calc(50% + 12px)}.map-panel:not(.comparing) .map-pair{grid-template-columns:1fr}.map-panel:not(.comparing) #raster-map,.map-panel:not(.comparing) .compare-title,.map-panel:not(.comparing) .legend{display:none}.map-badge{position:absolute;z-index:3;left:12px;bottom:42px;padding:7px 9px;background:rgba(23,37,31,.9);color:white;border-radius:8px}.legend{position:absolute;z-index:3;left:12px;bottom:12px;padding:7px 9px;background:rgba(255,253,247,.95);border:1px solid var(--line);border-radius:8px;font-size:10px}
...
</head><body><main id="shell"><header class="tabs" id="tabs"></header><section class="workspace"><aside class="panel queue"><div class="toolbar"><strong id="queue-title"></strong><span class="spacer"></span><button class="small" id="batch-accept">Batch accept</button><button class="small danger" id="batch-reject">Batch reject</button></div><div id="candidates"></div></aside><section class="panel map-panel" id="compare-panel"><div class="compare-title left">Structured evidence only</div><div class="compare-title right">Structured plus coarse raster</div><div class="map-pair"><div id="map" class="map"></div><div id="raster-map" class="map"></div></div><div class="map-badge" id="map-badge"></div><div class="legend">Same viewport · coarse surface is approximate and review-only</div></section><aside class="panel evidence" id="evidence"></aside></section><footer class="footer"><div><strong id="footer-title"></strong><div class="tiny muted" id="footer-state"></div></div><div class="progress"><span id="progress"></span></div><button class="primary" id="complete">Complete category</button></footer></main>
<script>
const tx=(zh,en)=>english?en:zh;const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
let desk=__DESK__,selected=new Set(),overlays=[],rasterOverlays=[],selectedRasterId=null,pending=false,syncingMaps=false;
const mapOptions={viewMode:'3D',zoom:__ZOOM__,pitch:__PITCH__,rotation:__ROTATION__,center:[__LNG__,__LAT__],showLabel:false};
const map=new AMap.Map('map',mapOptions),rasterMap=new AMap.Map('raster-map',mapOptions);
const boundary=__BOUNDARY__.map(p=>[p.lng,p.lat]);if(boundary.length>=3){[map,rasterMap].forEach(target=>target.add(new AMap.Polygon({path:boundary,strokeColor:'#17251f',strokeWeight:3,fillOpacity:0,zIndex:2})))}
const syncMap=(source,target)=>{if(syncingMaps)return;const a=source.getCenter(),b=target.getCenter(),same=Math.abs(a.lng-b.lng)<1e-9&&Math.abs(a.lat-b.lat)<1e-9&&source.getZoom()===target.getZoom()&&source.getPitch()===target.getPitch()&&source.getRotation()===target.getRotation();if(same)return;syncingMaps=true;target.setZoomAndCenter(source.getZoom(),a,true);target.setPitch(source.getPitch());target.setRotation(source.getRotation());queueMicrotask(()=>syncingMaps=false)};map.on('moveend',()=>syncMap(map,rasterMap));rasterMap.on('moveend',()=>syncMap(rasterMap,map));
const activeTab=()=>desk.categories.find(c=>c.id===desk.activeCategory);const activeCandidate=()=>desk.candidates.find(c=>c.id===desk.selectedCandidateId)||desk.candidates[0];const activeRaster=()=>desk.coarseRasterEvidence.find(r=>r.id===selectedRasterId)||null;const rasterPolygons=geometry=>geometry.type==='polygon'?[geometry.rings]:geometry.polygons;
function candidateOverlays(candidate,clickable){const color=candidate.disposition==='accepted'?'#2f765b':candidate.disposition==='rejected'?'#a54836':candidate.disposition==='deferred'?'#d08b32':'#315f78';return candidate.geometry.flatMap(path=>{if(!path.length)return[];const points=path.map(p=>[p.lng,p.lat]);let item;if(candidate.geometryForm==='point')item=new AMap.Marker({position:points[0],title:candidate.label,zIndex:candidate.id===desk.selectedCandidateId?120:20});else if(candidate.geometryForm==='centreline')item=new AMap.Polyline({path:points,strokeColor:color,strokeWeight:candidate.id===desk.selectedCandidateId?8:5,strokeStyle:candidate.disposition==='rejected'?'dashed':'solid',zIndex:candidate.id===desk.selectedCandidateId?120:20});else item=new AMap.Polygon({path:points,strokeColor:color,strokeWeight:candidate.id===desk.selectedCandidateId?7:4,strokeStyle:candidate.disposition==='rejected'?'dashed':'solid',fillColor:color,fillOpacity:candidate.disposition==='rejected'?.06:.18,zIndex:candidate.id===desk.selectedCandidateId?120:20});if(clickable)item.on('click',()=>selectCandidate(candidate.id));return[item]})}
function draw(){if(overlays.length)map.remove(overlays);if(rasterOverlays.length)rasterMap.remove(rasterOverlays);overlays=desk.candidates.flatMap(candidate=>candidateOverlays(candidate,true));rasterOverlays=desk.candidates.flatMap(candidate=>candidateOverlays(candidate,false));const raster=activeRaster();if(raster)rasterOverlays.push(...rasterPolygons(raster.approximateGeometry).flatMap(polygon=>polygon.length?[new AMap.Polygon({path:polygon.map(ring=>ring.map(p=>[p.lng,p.lat])),strokeColor:'#315f78',strokeWeight:4,strokeStyle:'dashed',fillColor:'#6f9eb7',fillOpacity:.32,zIndex:80})]:[]));if(overlays.length)map.add(overlays);if(rasterOverlays.length)rasterMap.add(rasterOverlays)}
function selectCandidate(id){selectedRasterId=null;desk.selectedCandidateId=id;post({type:'mapFoundationReviewCandidateSelected',category:desk.activeCategory,subjectId:id});render()}
...
function render(){const comparing=!!activeRaster();document.getElementById('compare-panel').classList.toggle('comparing',comparing);renderTabs();renderQueue();renderEvidence();renderFooter();document.getElementById('map-badge').textContent=(activeTab()?.label||desk.activeCategory)+' · '+tx('地图仅供空间上下文','spatial context only');draw();map.resize();if(comparing)rasterMap.resize()}
...
{editing_script}
    ...
    mod tests {
        ...
        use campus_tool_protocol::{
            MapAreaGeometry, MapBoundaryCandidate, MapBoundaryDesk, MapBoundaryDeskRequest,
            MapCoarseRasterEvidence, MapCoordinate, MapEvidenceAssessment,
            MapFoundationReviewCandidate, MapFoundationReviewCategory, MapFoundationReviewDesk,
            MapFoundationReviewDeskRequest, MapProviderOutcome,
        };
            ...
            ToolCommand::OpenFoundationReviewDesk {
                request: Box::new(MapFoundationReviewDeskRequest {
                    ...
                    desk: MapFoundationReviewDesk {
                        ...
                        known_gaps: Vec::new(),
                        coarse_raster_evidence: vec![MapCoarseRasterEvidence {
                            id: "raster-water-east-v1".into(),
                            linked_gap_id: "gap:water:osm:tile-1:0".into(),
                            ...
                            lineage_summary: "coarse-gap-water-v1 · vectorize-v1".into(),
                            exclusion_summary: "8 cells · structured geometry priority".into(),
                            assessment: MapEvidenceAssessment {
                                geometry: "approximate".into(),
                                semantics: "supported".into(),
                                entity_match: "not applicable".into(),
                                name_match: "not applicable".into(),
                            },
                            approximate_geometry: MapAreaGeometry::Polygon { rings: Vec::new() },
                            warnings: vec![
        ...
        fn map_tasks_expose_only_controls_for_the_current_job() {
            ...
            assert!(review_queue.contains("Refresh Dataset Bundle"));
            assert!(review_queue.contains("Structured evidence only"));
            assert!(review_queue.contains("Structured plus coarse raster"));
            assert!(review_queue
                .contains("Approximate coverage only; this is not a precise feature boundary."));
            assert!(review_queue.contains("mapCoarseRasterDecisionRequested"));
            assert!(review_queue.contains("mapCoarseRasterSupplementRequested"));
