...
> [!IMPORTANT]
> 当前版本为 **`2.0.0-dev`（实施期）**，不是可直接交付的稳定版本。核心业务模块已经实现并通过自动化测试；桌面端已接通边界和朝向步骤，采集页已有实现但正按 ADR-0037/0039 迁出壳内编排，评审与导出步骤仍在开发中。
...
| 数据采集 | 🟡 M3 hardening | 可插拔数据源、原始观测落库、增量变化检测、进度模型和采集页已有实现；B14 与 A1 `collection-flow` 尚未立户，69b5b15 不作为 M3 完成证据 |
...
- **39 份**已接受的 ADR（架构与产品决策记录）；
- **7 份**专题研究报告；
- **45 项**中英双语领域术语；
- **406 项**由 `cargo test --workspace` 执行的自动化测试（404 通过、2 忽略），当前全量测试通过。
...
架构规划目标为 31 个 crate；当前通过接口评审并加入 Workspace 的是 20 个。未加入 Workspace 的规划模块不视为已实现。
