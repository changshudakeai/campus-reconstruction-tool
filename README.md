...
> [!IMPORTANT]
> 当前版本为 **`2.0.0-dev`（实施期）**，不是可直接交付的稳定版本。核心业务模块已经实现并通过自动化测试；桌面端已接通边界和朝向步骤，采集页已有实现但正按 ADR-0037/0039/0040 迁出壳内编排并补齐候选资格契约，评审与导出步骤仍在开发中。
...
| 数据采集 | 🟡 M3 hardening | 可插拔数据源、原始观测落库、增量变化检测、进度模型和采集页已有实现；`36955e2` 已建立 B14 代码基础，但缺少模块文档导致 workspace test/xtask ci 未过，且点/线/面契约、候选投影、资格查询与 A1 `collection-flow` 尚未完成 |
...
- **40 份**已接受的 ADR（架构与产品决策记录）；
- **7 份**专题研究报告；
- **47 项**中英双语领域术语；
- 自动化测试覆盖各模块与架构规则；当前 `cargo test --workspace` 和 `cargo xtask ci` 被 B14 `validator.rs` 缺少 `//!` 模块文档阻断，其余本轮已执行门禁通过，不能宣称全量绿灯。
...
架构规划目标为 31 个 crate；当前通过接口评审并加入 Workspace 的是 21 个。未加入 Workspace 的规划模块不视为已实现。
