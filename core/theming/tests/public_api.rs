...
fn public_api_types_exist() {
    ...
    let card = theming::ColorCard {
        name: "Test".into(),
        colors,
    };
    assert_eq!(
        card.get(theming::ColorRole::PrimaryBackground),
        Some("#FFFFFF")
    );
    ...
    let table = theming::MotionTable {
        fast: theming::DurationConfig {
            duration: 0.2,
            easing: theming::EasingType::Linear,
        },
        medium: theming::DurationConfig {
            duration: 0.5,
            easing: theming::EasingType::EaseOut,
        },
        slow: theming::DurationConfig {
            duration: 1.0,
            easing: theming::EasingType::EaseInOut,
        },
    };
    ...
    assert!(manager
        .active_colors()
        .get(theming::ColorRole::PrimaryBackground)
        .is_some());
