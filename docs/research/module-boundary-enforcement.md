...
Status: research-reference（非规范）

> 本文用于解释工具选型，不得覆盖当前规则。规划模块总数已重新核算为 27；功能横向依赖改为默认禁止、显式白名单例外；crate 源文件数量不再设硬上限。现行规则见 ADR-0017 与 `docs/developer-guide/enforcement.md`。
