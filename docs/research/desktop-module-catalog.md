...
> 本文是早期研究，不是当前实施规则。涉及最小导出、朝向必选、模块数量或依赖边的结论已可能失效；当前产品见 `docs/product-baseline.md`，当前架构见 ADR-0017/0037/0039/0041。

> **后续决策（2026-08-01）**：本文是 ADR-0037/0039 之前的候选目录研究。文中的“S1 业务编排”已经失效；S1 现在只承担呈现与构造期接线，跨功能采集操作由 A1 `collection-flow` 拥有。模块职责与本文冲突时以 ADR-0037/0039 和 `docs/module-decisions.md` 为准。
...
- **定义（已由 ADR-0037 修订）**：唯一叶子 crate，负责把功能模块拼成可运行应用（UI 声明 + 构造期注入）；运行期业务编排不属于 S1。
...
```
apps/desktop (叶子：.slint UI + serde + 构造期注入；无运行期业务编排)
...
| 主程序应用壳 | `apps/desktop` | 应用壳 | ADR-0001/0037 | .slint 生成、构造期注入、用户入口；无运行期业务编排 |
...
| # | 模块名（中文 + crate） | 类型 | 职责 | 是否待访谈 |
|---|-----------------------|------|------|-----------|
| S1 | 主程序应用壳 (`apps/desktop`) | 应用壳 | .slint UI 声明、构造期注入、用户入口、快捷方式生成；无运行期业务编排 | 已由 ADR-0037 定案 |
