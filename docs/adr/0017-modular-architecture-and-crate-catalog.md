...
已接受（2026-07-25，产品负责人确认；同日被 ADR-0022/0024 调整；2026-07-30 由 ADR-0037 收紧 S1；2026-08-01 由 ADR-0039/0040 补充应用流程与候选资格；同日取消“单 crate 不超过 10 个源文件”硬上限，并将横向依赖改为默认禁止、显式白名单例外）
...
| 2 | **低耦合** | 功能模块默认不直接依赖；跨功能协作优先通过应用流程或能力端口；必要直接边必须有决策记录并命中白名单 | `cargo metadata` 无环、横向边仅命中白名单 |
...
| 10 | **粒度适中** | 单文件 1000 行为带理由豁免的警戒线；不按 crate 文件数量设硬上限，改查职责内聚、公共 API、依赖扇出和编译时间 | tidy + public-api 快照 + 架构图 + timings + 评审清单 |

---

### 二、最终模块目录清单（27 个 crate，按本节实际目录重新核算）
...
| B14 | 几何验证器 | `geometry-validator` | 按点/线/面种类验证采集候选几何，记录唯一修复与隔离结果；**内含 R-tree 空间索引子模块** | ADR-0032/0040；`36955e2` 已建立代码基础但全量门禁未闭合，尚待形状感知接口和候选资格接入 |
...
**关键约束**：
1. **横向依赖**：功能模块之间默认不直接依赖（如当前 F4→F7 不获批准）；必要直接边必须证明职责仍内聚、方向单向、接口稳定，并由决策记录加入架构白名单。基础模块同样默认零依赖，只允许已批准单向边；
...
6. **候选资格**：原始观测与评审候选分层；B2 持久化可重建候选投影并只向 F5 提供“可评审”集合，F9 只消费 F5 封账后的保留候选，任何模块不得从原始观测绕过 B14 重建自己的可见性规则（ADR-0040）。
...
| crate 源文件数量 | 不设硬阈值 | 任意 | 不因文件数量阻断；禁止为凑数合并无关代码或机械拆 crate |
| 循环依赖 | Cargo 原生拒绝（crate 级循环无法构建，无需额外工具） | 出现环 | 编译失败 |
| 同层横向依赖/分层违规 | xtask 架构测试（cargo metadata 断言白名单与分层） | 未登记横向边/底座反依/壳越白名单 | CI 阻断 |
