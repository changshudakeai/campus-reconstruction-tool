...
Cutover requires all items in a maintained functional parity matrix to pass; successful migration of existing projects; complete real-campus runs through Foundation and Detailed modes; verified map, native preview, editing, and export; installation, uninstallation, recovery, and offline checks; an installer below the 50 MB budget; and no blocking crash during an extended workflow. The current desktop entry point changes only after automated evidence and final user experience acceptance.
