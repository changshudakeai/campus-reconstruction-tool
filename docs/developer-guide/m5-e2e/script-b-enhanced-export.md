...
状态：M5 实施窗口准备（2026-08-06）。真实高德在线链路为 M5 唯一强制人工
环节：本地演示密钥返回 `USERKEY_PLAT_NOMATCH`（密钥平台/域名未对本机放行），
需产品负责人提供已授权的 Web 端(JS API) 密钥或约定现场验收时间/环境。
...
| 2 确认边界 | 打开方案 → 地图圈边界 → 确认 | 步骤①完成 |
| 3 候选采集 | 进入采集页 → 点"采集"（真实高德在线） | 进度逐类别推进；完成后显示采集报告（原始/可评审/修复/隔离计数） |
