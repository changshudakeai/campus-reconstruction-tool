...
crate 的 `src/` 文件数量不设硬上限。文件多不等于职责散乱，文件少也不等于内聚；禁止为了通过计数门禁把无关代码塞进大文件或机械拆出浅 crate。新增模块、公共 API 或依赖边时必须核对：
...
正式发布前的本地开发数据库不承诺 schema 向后兼容；schema 基线变化后删除并重建。正式发布后迁移改为只追加、不可改写。
...
- `架构违规: 禁止边 data-acquisition → review-workbench：功能模块横向边未命中白名单（ADR-0017）`
